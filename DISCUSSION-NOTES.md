...
> Mobile refinement: the board fills the viewport. Decimal placement uses a draggable gold caret on the bar. Retain the live number and compact Clear/Settings controls. Remove the target-number field, undo button, instructional captions, and footer. Move sound, shake, and help into Settings; show the logo in a dismissible first-visit welcome. Use short contact sounds without repeated sliding tones. Report motion permission blocks and missing sensor data with the steps that belong to the browser in hand, since the setting lives in a different place on each platform. This refinement supersedes earlier presentation decisions below.

> Current scope (September 19, 2026): mobile-first free play, client-only PWA, published publicly as Remco28/abacus on GitHub Pages at abacus.teamremco.org. Real rod-constrained bead physics, simultaneous touch, counted-bead highlights, traditional dots every third rod, adjustable ones position and beginner place labels. Sound defaults on; three-shake reset is core. Optional manually entered practice number, no challenge mode or exercise generator (superseded September 23, 2026 by practice mode: a generated addition, subtraction or multiplication problem, read in a dialog and answered on the board, with tiers following the classical soroban order). Difficulty is two ladders rather than one scale (September 23, 2026): adding and taking away is graded by the bead movement a step needs — direct, small friends, big friends, double combination, then rows — over eleven rungs, and multiplying by operand width over six, from 1 × 1 to 3 × 3. In the problem dialog the answer rule is drawn only when the answer is, because a line with nothing written under it reads as a fault. In a column of three or more numbers the learner can mark their place by tapping a line in the dialog, and the mark is saved with the problem; it is offered only inside the dialog, and only where there is a middle to lose, so multiplying never shows it. Landscape drops the place-value names in favour of taller beads (September 23, 2026). The reckoning bar clears the board (September 23, 2026): while the decimal caret is locked, a sideways sweep of the bar sends each rod's beads away from it as the finger reaches them, a sweep to the far edge clears the board, and lifting early springs the beads back, so a graze cannot wipe a number in progress; the caret keeps the bar while it is unlocked, so the two gestures never compete. Settings is grouped into Board, Practice and Help (September 23, 2026), with the worksheet first in Practice, and the on-board drill renamed from Test mode to Solve on the board so the two ways of practising read as peers. The same generator also drives a worksheet (September 23, 2026): a numbered sheet of problems at the chosen rungs, for a soroban in front of you rather than the one on screen, so it needs no test mode and leaves the board untouched. Sheet length is per rung, measured against the distinct problems each rung can actually produce — about 26 on the smallest one-digit rung, a few hundred across the other one-digit rungs, thousands above that — so a sheet is short at the bottom of a ladder and long at the top, and a sheet never repeats a problem. One problem is checked at a time, the sheet is saved with its checked problems, and printing sets it in two columns with the board left off the page. Motion guidance is chosen from the browser rather than written once for all of them (September 23, 2026), because Android keeps a per-site Motion sensors permission, iOS keeps Motion & Orientation Access in Safari's settings, and a laptop has no sensor; Chrome also fires empty devicemotion events when that permission is blocked, so three of those are reported as a blocked permission immediately instead of waiting out the no-data grace period. Desktop tutorials are deferred. Earlier proposals below are brainstorming history; this paragraph and README.md supersede conflicting MVP suggestions.
