...
Hold the gold caret for half a second until it brightens and enlarges, then drag horizontally along the counting bar to move the decimal. Release to lock it again. Moving more than eight screen pixels before the hold completes cancels it; track taps and place labels do not change the decimal. A first-use hint explains the hold. Keyboard arrows remain available without a hold. Traditional marker dots stay fixed every third rod. In portrait, the number, Clear, and Settings sit above the board. Landscape moves them to a slim right-hand strip and puts the place labels below the beads, leaving a buffer above the phone's gesture edge. A dismissible logo welcome appears on the first visit.
