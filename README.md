...
Tap the lock beside Clear to unlock the gold caret, then drag horizontally along the counting bar to move the decimal. Releasing locks it again; while locked, touches over the caret and track do nothing. Place labels are read-only. Keyboard arrows work after unlocking. Traditional marker dots stay fixed every third rod. In portrait, the number, Clear, and Settings sit above the board. Landscape moves them to a slim right-hand strip and puts the place labels below the beads, leaving a buffer above the phone's gesture edge. A dismissible logo welcome appears on the first visit.

Settings also holds Test mode. Choose the operations to practise (addition, subtraction, multiplication) and a level, then press Problem to read a problem in a dialog, work it out on the beads, and press Submit. A wrong answer offers to clear the board, keep it, or reveal the answer. Test mode holds the decimal at four whole-number places so nothing it asks for can leave the abacus, and it reports the verdict through the same live region a screen reader already follows. The problem you are on survives a reload.
...
`npm test` covers digits 0–9, contiguous counting, decimal precision, bead pushing, collision energy/constraints, shake timing/debounce, and the practice-mode generator: the complement classifier, the level rules, the board's capacity, and subtraction that never passes through a negative total. `node tests/browser.mjs <CDP HTTP endpoint> <app URL>` checks simultaneous touch, drag/keyboard decimal control, persistence, first-visit welcome dismissal, shake reset, denied/no-data sensor guidance, and landscape layout. `node tests/browser-testmode.mjs <CDP HTTP endpoint> <app URL>` checks practice mode end to end: normalizing and holding the decimal, reading a problem in its dialog, clear/keep/reveal on a wrong answer, accepting and rejecting answers, restoring the decimal on exit, and the problem and board surviving a reload. Synthetic motion/pointer tests do not replace physical iPhone and Android validation.
