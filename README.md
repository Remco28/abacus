...
Settings is grouped into Board, Practice, and Help. Practice holds the two ways to work, and they share one set of choices: the operations to practise (addition, subtraction, multiplication, division) and a level on each ladder. Adding and taking away is graded by the bead movement a step needs, multiplying by operand width, and division by divisor and quotient size. Division problems are exact whole-number divisions; remainders are not generated. Help also opens a picture-first tutorial with its own three-rod practice board, short demonstrations, and bead-by-bead challenges for all four operations. Learners can follow the suggested add → subtract → multiply → divide sequence or jump directly to any operation and lesson.
